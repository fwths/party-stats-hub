...
This ADR reflects the direction of the isolated Character Schema V3 work. It is not authoritative until the team accepts it after the V3 persistence and migration gates are complete.
...
The aggregate stores authored and mutable facts. A compiled character sheet is derived, disposable, and reproducible from the aggregate plus exact versioned rule content.

Exact rule references, unresolved import decisions, owner authorization, build/live revisions, and audit events are part of the proposed integrity boundary.
...
Campaign membership is therefore optional character metadata. The current V3 experiment requires a `campaignId`; that requirement must become optional, or this ADR must be revised, before acceptance and production persistence.
