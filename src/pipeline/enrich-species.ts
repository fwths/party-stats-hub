...
async function enrich() {
  ...
  for (const sp of speciesList) {
    ...
    const match = races.find(r => r.name.toLowerCase() === ddbName || ddbName.includes(r.name.toLowerCase()));
    ...
    if (match) {
      ...
      if (!sp.abilityScoreIncreasesJson) {
         if (match.ability && match.ability.length > 0) {
            ...
            didUpdate = true;
         } else if (match.lineage || ["hexblood", "dhampir", "reborn", "lineage"].some(l => ddbName.includes(l))) {
      ...
      // Parse Languages
      if (!sp.languagesJson && match.languageProficiencies && match.languageProficiencies.length > 0) {
         const langObj = match.languageProficiencies[0];
         const parsedLangs = Object.keys(langObj).filter(k => k !== "any" && k !== "other").map(k => k.charAt(0).toUpperCase() + k.slice(1));
      ...
      if (didUpdate) {
        ...
        if (!sp.abilityScoreIncreasesJson && ["hexblood", "dhampir", "reborn", "lineage"].some(l => ddbName.includes(l))) {
            await db.update(schema.species)
