...
export async function getNotionData(token: string, searchParams: URLSearchParams) {
  ...
  if (pageId) {
    ...
    if (!blocksResponse.ok) {
      ...
      throw new Error(errData.message || "Failed to fetch content from Notion.");
    }
    ...
    return { success: true, markdown, parentDb };
  }
  ...
  if (
    ...
    if (parentType === "page") {
      ...
      if (!blocksResponse.ok) {
        const errData = await blocksResponse.json().catch(() => ({}));
        throw new Error(errData.message || "Failed to fetch page blocks from Notion.");
      }
