    ...
    handlers: {
      ...
      GET: async ({ request }) => {
        try {
          ...
          if (pageId) {
            ...
            const fetchAndFormatDatabase = async () => {
              ...
              if (dbResponse.ok) {
                ...
                try {
                  const dbInfoResponse = await fetch(`https://api.notion.com/v1/databases/${pageId}`, {
                    method: "GET",
                    headers: {
                      "Authorization": `Bearer ${token}`,
                      "Notion-Version": "2022-06-28",
            ...
            const blocksResponsePromise = fetch(`https://api.notion.com/v1/blocks/${pageId}/children`, {
              method: "GET",
              headers: {
                "Authorization": `Bearer ${token}`,
                "Notion-Version": "2022-06-28",
            ...
            if (!blocksResponse.ok) {
              ...
              return new Response(
                JSON.stringify({ error: errData.message || "Failed to fetch content from Notion." }),
          ...
          if (parentId && parentId !== "workspace" && parentId !== "undefined" && parentId !== "null" && parentId.trim() !== "") {
            if (parentType === "page") {
              const blocksResponse = await fetch(`https://api.notion.com/v1/blocks/${parentId}/children`, {
                method: "GET",
                headers: {
                  "Authorization": `Bearer ${token}`,
                  "Notion-Version": "2022-06-28",
          ...
          const hasNoParent = !parentId || parentId === "workspace" || parentId === "undefined" || parentId === "null" || parentId.trim() === "";
          ...
          if (workspaceSearch || hasNoParent || searchQuery !== null) {
            ...
            while (hasMore) {
              ...
              if (!searchResponse.ok) {
                ...
                return new Response(
                  JSON.stringify({ error: errData.message || "Failed to search Notion workspace." }),
...
function getPropertyValueText(prop: any): string {
  ...
  if (Array.isArray(prop)) {
    return prop.map(item => getPropertyValueText(item)).filter(Boolean).join(", ");
  }
  ...
  switch (type) {
    ...
    case "rich_text":
      ...
      const hasMentions = (val || []).some((t: any) => t.type === "mention" && t.mention?.type === "page");
      if (hasMentions) {
        return (val || [])
          .map((t: any) => {
    ...
    case "rollup":
      ...
      if (val && val.type === "array" && Array.isArray(val.array)) {
        return val.array.map((item: any) => getPropertyValueText(item)).filter(Boolean).join(", ");
      }
...
function sortDatabaseResults(results: any[], databaseTitle: string): any[] {
  ...
  if (titleKey) {
    return [...results].sort((a, b) => {
      ...
      const textA = (propA?.title || []).map((t: any) => t.plain_text || "").join("").toLowerCase();
      const textB = (propB?.title || []).map((t: any) => t.plain_text || "").join("").toLowerCase();
...
function sortColumnKeys(columnKeys: string[], dbTitle: string): string[] {
  ...
  const matchKey = Object.keys(PREFERRED_COLUMN_ORDERS).find((key) =>
