...
This is a candidate model for a future mature multi-user campaign system. Current campaign code does not yet implement the complete transfer, archival, recovery, and authorization workflows described here, so auditors must not treat their absence as a release defect unless the behavior is already exposed or committed for the near term.
