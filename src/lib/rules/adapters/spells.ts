...
import { getCantripLimit, getPreparedSpellLimit, getMaxSpellLevel, getSpellcastingInfo } from "../../../components/builder/BuilderUtils";
...
export function spellcastingToRuleChoicesAndGrants(
  ...
  if (preparedLimit > 0) {
    choices.push({
      ...
      prerequisites: [
        { type: "class", value: classData.id },
        { type: "level", value: maxSpellLevel } // Indicates max level of spell that can be prepared
      ],
      provenance,
    });
  }
