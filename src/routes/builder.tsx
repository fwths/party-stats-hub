  ...
  return (
    <div className="container mx-auto p-4 md:p-8 min-h-screen animate-in fade-in duration-700">
      ...
      <div className="flex items-center gap-4 mb-10 relative">
        ...
        <Link to="/">
          <Button variant="outline" size="sm" className="backdrop-blur-sm bg-background/50 border-border/50 hover:bg-primary/20 hover:border-primary/50 transition-all">
        ...
        {[
          ...
          <div 
            ...
            className={`flex-1 flex flex-col items-center p-3 rounded-xl transition-all duration-500 relative overflow-hidden ${
              step === s.id ? "bg-primary/15 shadow-[0_0_20px_rgba(var(--primary),0.2)] text-primary scale-105" : 
              step > s.id ? "bg-secondary/30 text-foreground/80 cursor-pointer hover:bg-secondary/50" : "opacity-40 grayscale"
            }`}
          ...
          >
            ...
            <s.icon className={`h-7 w-7 mb-2 transition-transform duration-300 ${step === s.id ? "scale-110 drop-shadow-[0_0_8px_rgba(var(--primary),0.8)]" : ""}`} />
            ...
            {step === s.id && <div className="absolute bottom-0 left-0 w-full h-1 bg-primary shadow-[0_0_10px_var(--primary)]" />}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 max-w-4xl mx-auto">
      <div className="bg-secondary/20 p-6 rounded-xl border border-border/30">
        <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">What is your name, hero?</label>
        <input 
          type="text" 
          ...
          {races.map(race => (
            ...
            >
              ...
              <CardHeader className="pb-3">
                <CardTitle className="text-2xl font-heading group-hover:text-primary transition-colors">{race.name}</CardTitle>
              </CardHeader>
              <CardContent className="relative z-10">
                <p className="text-sm text-muted-foreground leading-relaxed min-h-[60px]">{race.description}</p>
                <div className="mt-5 flex flex-wrap gap-2 pt-4 border-t border-border/30">
                  {race.abilityBonuses.map((b: any) => (
                    <span key={b.ability} className="text-xs font-bold bg-accent/10 border border-accent/20 text-accent px-2.5 py-1 rounded-md shadow-sm">
                      +{b.bonus} {b.ability}
                    </span>
                  ))}
                </div>
      ...
      {character.raceId && races.find(r => r.id === character.raceId)?.subraces && (
        <div className="mt-8 bg-secondary/20 p-6 rounded-xl border border-border/30 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Choose Subrace / Lineage</label>
            ...
            >
              <SelectTrigger className="w-full bg-background/50 border-border/50 focus:ring-primary shadow-sm h-12">
                <SelectValue placeholder="Select a subrace...">
                  {character.subraceId 
                    ? races.find(r => r.id === character.raceId)?.subraces?.find(s => s.id === character.subraceId)?.name 
                    : undefined}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
                {races.find(r => r.id === character.raceId)?.subraces?.map((sub) => (
                  <SelectItem key={sub.id} value={sub.id} className="py-3 focus:bg-primary/10 cursor-pointer">
                    <div className="font-bold text-primary">{sub.name}</div>
                    <div className="text-xs text-muted-foreground mt-1 max-w-xs whitespace-normal">{sub.description}</div>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
...
function StepClass({ character, updateCharacter }: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {classes.map(cls => (
            ...
            >
              ...
              <CardHeader className="pb-3">
                <CardTitle className="text-2xl font-heading group-hover:text-amber-500 transition-colors">{cls.name}</CardTitle>
              </CardHeader>
              <CardContent className="relative z-10">
                <p className="text-sm text-muted-foreground leading-relaxed min-h-[60px]">{cls.description}</p>
                <div className="mt-5 flex gap-4 text-xs pt-4 border-t border-border/30">
                  <div className="bg-secondary/50 px-2.5 py-1.5 rounded-md"><span className="font-bold text-foreground">Hit Dice:</span> <span className="text-amber-500">d{cls.hitDice}</span></div>
                  <div className="bg-secondary/50 px-2.5 py-1.5 rounded-md"><span className="font-bold text-foreground">Saves:</span> <span className="text-amber-500">{cls.saves.join(", ")}</span></div>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      {character.classId && (
        <div className="mt-8 bg-secondary/20 p-6 rounded-xl border border-border/30 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Starting Level</label>
          ...
          {character.level >= 3 && classes.find(c => c.id === character.classId)?.subclasses && (
            <div className="mt-8 pt-6 border-t border-border/30 animate-in fade-in slide-in-from-bottom-2 duration-500">
              <label className="block text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3 text-amber-500">Choose Subclass / Path (Level 3+)</label>
              <div className="max-w-md">
                <Select 
                  ...
                  <SelectTrigger className="w-full bg-background/50 border-amber-500/30 focus:ring-amber-500 shadow-sm h-12">
                    <SelectValue placeholder="Select a subclass...">
                      {character.subclassId 
                        ? classes.find(c => c.id === character.classId)?.subclasses?.find(s => s.id === character.subclassId)?.name
                        : undefined}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
                    {classes.find(c => c.id === character.classId)?.subclasses?.map((sub) => (
                      <SelectItem key={sub.id} value={sub.id} className="py-3 focus:bg-amber-500/10 cursor-pointer">
                        <div className="font-bold text-amber-500">{sub.name}</div>
                        <div className="text-xs text-muted-foreground/80 mt-1 max-w-xs whitespace-normal group-focus:text-muted-foreground">{sub.description}</div>
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
...
function StepAbilities({ character, updateCharacter }: any) {
  ...
  const getAvailableArrayValues = (currentStatAb: string) => {
    ...
    return standardArray.filter(v => !used.includes(v) || character.abilities[currentStatAb] === v);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 max-w-4xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-muted-foreground">Choose your generation method. Racial bonuses from your heritage are applied automatically.</p>
      </div>

      <div className="flex justify-center gap-4 mb-8">
        <Button 
          variant={method === "standard" ? "default" : "outline"} 
          onClick={() => { setMethod("standard"); setStandardArrayDefaults(); }}
      ...
      {method === "pointbuy" && (
        <div className="text-center mb-8 bg-secondary/30 py-3 rounded-lg border border-border/50 max-w-sm mx-auto">
          <span className="text-sm font-bold uppercase tracking-widest text-muted-foreground block mb-1">Points Remaining</span>
          <span className={`text-3xl font-black ${27 - getPointsUsed() < 0 ? "text-destructive" : "text-emerald-500"}`}>
            {27 - getPointsUsed()}
          </span>
      ...
      {method === "roll" && (
        <div className="flex justify-center mb-8">
          <Button onClick={rollAll} size="lg" className="bg-amber-600 hover:bg-amber-700 text-white font-bold gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {abilitiesList.map(ab => {
          ...
          return (
            <div key={ab} className={`relative bg-card/50 p-6 rounded-2xl border flex flex-col items-center transition-all duration-300 hover:-translate-y-1 ${isHigh ? 'border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.1)]' : 'border-border/40 hover:border-emerald-500/30 hover:shadow-md'}`}>
              {isHigh && <div className="absolute top-0 right-0 w-16 h-16 bg-emerald-500/10 blur-xl rounded-full" />}
              
              <span className="font-bold text-sm tracking-widest uppercase text-muted-foreground mb-3">{ab}</span>
              
              <div className={`text-5xl font-black mb-4 tracking-tighter drop-shadow-md ${isHigh ? 'text-emerald-400' : 'text-foreground'}`}>
                {base === 0 ? "-" : total}
              </div>
              
              <div className={`text-base font-bold px-4 py-1.5 rounded-full mb-6 border shadow-sm ${mod > 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : mod < 0 ? 'bg-destructive/10 text-destructive border-destructive/20' : 'bg-secondary text-foreground border-border'}`}>
                {base === 0 ? "-" : mod > 0 ? `+${mod}` : mod}
              </div>
              
              <div className="w-full mt-auto space-y-3 pt-4 border-t border-border/30">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground font-semibold uppercase tracking-wider">Base</span>
                  ...
                  {method === "standard" ? (
                    ...
                    >
                      ...
                      <SelectContent className="min-w-[4rem]">
                        <SelectItem value="0" className="justify-center font-mono font-bold">-</SelectItem>
                        {standardArray.map(v => {
                          const usedElsewhere = abilitiesList.some(otherAb => otherAb !== ab && character.abilities[otherAb] === v);
                          if (!usedElsewhere || base === v) {
                            return <SelectItem key={v} value={v.toString()} className="justify-center font-mono font-bold">{v}</SelectItem>;
                          }
                          return null;
                        })}
                      </SelectContent>
                ...
                {racial > 0 && (
                  <div className="flex items-center justify-between text-xs animate-in fade-in zoom-in duration-500">
                    <span className="text-accent font-semibold uppercase tracking-wider">Racial</span>
                    <span className="bg-accent/20 text-accent px-2 py-0.5 rounded font-bold">+{racial}</span>
                  </div>
                )}
              </div>
            </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 max-w-3xl mx-auto">
      <div className="text-center p-8 bg-gradient-to-b from-primary/10 to-transparent rounded-3xl border border-primary/20 relative overflow-hidden">
        ...
        <h2 className="text-5xl font-black tracking-tight bg-gradient-to-br from-white to-primary/50 bg-clip-text text-transparent mb-3 drop-shadow-sm">{character.name}</h2>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3 my-8">
        {Object.entries(character.abilities).map(([ab, base]: [string, any]) => {
          ...
          return (
            <div key={ab} className="bg-card/60 backdrop-blur-sm border border-border/40 p-3 rounded-xl flex flex-col items-center justify-center relative overflow-hidden shadow-sm">
              <div className="text-[10px] font-bold tracking-widest uppercase text-muted-foreground mb-1">{ab}</div>
              <div className="text-2xl font-black text-foreground">{total}</div>
              <div className={`absolute bottom-0 left-0 w-full h-1 ${mod > 0 ? 'bg-emerald-500/50' : mod < 0 ? 'bg-destructive/50' : 'bg-transparent'}`} />
            </div>
          );
        })}
      </div>
        ...
        <div className="flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              When you save, this character will be forged natively and instantly materialized into your party roster. You can manage their health, inventory, and stats from the main dashboard.
            </p>
          </div>
        </div>
      </div>
