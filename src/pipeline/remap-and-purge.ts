...
async function run() {
  const mpmmSpecies = [
    ...
    "fire genasi", "water genasi", "githyanki", "githzerai", "goblin", 
    "goliath", "harengon", "hobgoblin", "kenku", "kobold", "lizardfolk", 
    "minotaur", "orc", "satyr", "sea elf", "shadar-kai", "shifter", 
    "tabaxi", "tortle", "triton", "yuan-ti"
  ];
  
  const speciesList = db.select().from(schema.species).all();
  
  for (const s of speciesList) {
    if (mpmmSpecies.includes(s.name.toLowerCase()) && s.source !== "Mordenkainen Presents: Monsters of the Multiverse" && !s.id.includes("2024")) {
