import { PartyMember, SenseInfo, SpellSlotLevel, ActionInfo, DeathSaves, DefenseInfo } from "./dndbeyond.functions";
...
export function getLocalArmorModel(memberId: number, initialArmorModel: string | null): string | null {
...
export function getFullyModifiedStats(member: PartyMember) {
  ...
  const { spellSlots, pactSlots } = getLocalSpellSlots(member.id, member.spellSlots, member.pactSlots);
...
export function getModifiedDefenses(
  ...
  if (isBarbarian && isRaging) {
    ...
    physicalTypes.forEach((dt) => {
      if (!list.some((d) => d.damageType.toLowerCase() === dt.toLowerCase() && d.type === "resistance")) {
...
export function getModifiedHitDice(hitDiceStr: string, spentHitDice: Record<string, number>): string {
