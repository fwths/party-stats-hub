...
function renderEntry(entry: any): string {
  ...
  if (entry.type === "entries" || Array.isArray(entry.entries)) {
    ...
    return entry.name ? `<div><strong>${cleanTags(entry.name)}.</strong><br/>${content}</div>` : content;
  }
...
async function run() {
  ...
  for (const race of xphbRaces) {
    ...
    const fallbackDesc = legacySpecies?.description || cleanTags(race.entries?.[0]?.entries?.[0] || "");
